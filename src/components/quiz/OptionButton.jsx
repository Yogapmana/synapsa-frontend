...
export function OptionButton({
  ...
  if (!isRevealed) {
    buttonClass += 'border-[var(--border)] hover:border-green-500 hover:bg-tertiary/10 bg-surface cursor-pointer';
  } else if (isCorrectSelected || isMissedCorrect) {
    buttonClass += 'border-green-500 bg-tertiary/10 cursor-default';
  } else if (isWrongSelected) {
    buttonClass += `border-red-500 bg-red-50 cursor-default ${!shouldReduceMotion ? 'animate-shake' : ''}`;
  } else {
    buttonClass += 'border-[var(--border)] bg-neutral opacity-60 cursor-default';
  }
    ...
    >
      ...
      {isRevealed && (isCorrectSelected || isMissedCorrect) && (
        <motion.div
          ...
          className="w-6 h-6 rounded-full bg-tertiary flex items-center justify-center flex-shrink-0"
        >
